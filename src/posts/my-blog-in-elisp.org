...
For instance, I was then able to also very elegantly add an automatically generated list of all the posts in the =index.org=, by first generating a list of them in =build.el=, and then generating the html based on that list via a single ~elisp~ code block in =index.org=:
...
#+begin_src emacs-lisp :filename build.el
  ...
  (defconst
    blog/all-posts
    (let* ((posts-dir (expand-file-name "./src/posts"))
           (org-files (directory-files posts-dir t "\\.org$")))
      ;; ... Process the files and generate the list of their metadata (title, ...).
    )
  )
...
#+begin_src emacs-lisp
  (let ((posts-sorted (sort (copy-sequence blog/all-posts)
...
* Footnotes
[fn:1] https://github.com/Martinsos/blog-in-emacs-lisp/tree/faf7c1dda74c72bb8a99a5b679af0b87e3fcf21d
